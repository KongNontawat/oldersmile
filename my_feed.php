...
WHERE p.cat_id IN (".implode(',',$my_cat).")
GROUP BY p.post_id
...
<body>
        ...
        <div class="row gy-3 mt-2 mt-lg-3">
            ...
            <?php include('component/banner.php'); ?>
                ...
                <nav class="nav nav-pills nav-fill nav-justified mb-3 tab">
                    <a href="index.php" class="nav-link border bg-white">โพสต์ทั้งหมด</a>
                    <a href="my_feed.php" class="nav-link border active">โพสต์ที่สนใจ</a>
                </nav>

                <h4 class="active-header mb-2 mb-lg-3 d-md-none"></h4>

                <?php if(isset($_GET['cat_name'])): ?>
                <h4 class="mb-2 mb-lg-3">เรื่องที่สนใจเรื่อง : <?php echo $_GET['cat_name']; ?></h4>
                <?php endif; ?>
    ...
    <script>
        $(function() {
            ...
            $('.active-header').text('โพสต์ที่สนใจ');
